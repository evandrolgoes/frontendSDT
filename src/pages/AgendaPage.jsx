...
function emptyForm() {
  return { titulo:"", data_inicio:"", data_fim:"", hora_inicio:"09:00", hora_fim:"10:00", descricao:"", local:"", dia_todo:false, com_meet:false, convidados:[] };
}
...
export function AgendaPage() {
  ...
  useEffect(() => {
    api.get("/agenda-configs/").then(({ data }) => {
      ...
      setConfigs(conn);
      if (conn.length>0) setSelectedConfig(conn[0].id);
    }).catch(()=>{});
  }, []);

  // load events on config/view/date change
  useEffect(() => { if (selectedConfig) loadEventos(); }, [selectedConfig, cur, view]);
  ...
  const loadEventos = async () => {
    ...
    try {
      const {data} = await api.get(`/agenda/eventos/?config_id=${selectedConfig}&data_inicio=${inicio}&data_fim=${fim}`);
  ...
  const executeSave = async (payload, enviarConvites) => {
    ...
    try {
      const body = { ...payload, config_id: selectedConfig, enviar_convites: enviarConvites };
      if (editingEvId) {
        await api.put("/agenda/eventos/", { ...body, event_id: editingEvId });
      } else {
        await api.post("/agenda/eventos/", body);
      }
      setShowForm(false);
      setForm(emptyForm());
      setEditingEvId(null);
      ...
      loadEventos();
    } catch(err) { setError(err?.response?.data?.detail||"Erro ao salvar evento."); }
  ...
  const saveDraggedEvent = async (dragged) => {
    ...
    setError("");
    try {
      await api.put("/agenda/eventos/", { ...body, config_id: selectedConfig, event_id: dragged.ev.id, enviar_convites: false });
  ...
  if (!selectedConfig && configs.length===0) return (
  ...
  const sidebarEvents = selectedDay ? eventsForDay(eventos, selectedDay) : eventos.slice().sort(compareEvents);
  const sidebarTitle = selectedDay ? `Atividades de ${fmtDateLong(selectedDay)}` : "Atividades da agenda";
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",width:"100%",minWidth:0,flex:1,alignSelf:"stretch",fontFamily:"Arial,sans-serif",background:"#fff",overflow:"hidden"}}>
      ...
      <div style={S.topBar}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <span style={{fontSize:20,fontWeight:400,color:"#3c4043",letterSpacing:-.5}}>Agenda</span>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {configs.length>1 && (
            <select value={selectedConfig||""} onChange={e=>setSelectedConfig(Number(e.target.value))} style={S.sel}>
              {configs.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}
            </select>
          )}
                    ...
                    {weekDays.map((day,di)=>{
                      ...
                      return (
                        <div key={di} style={{flex:1,position:"relative",borderLeft:di>0?"1px solid #dadce0":"none"}}>
                          {dayEvs.map(ev=>{
                            const isDraggingThis = dragState?.ev?.id===ev.id;
...
function AgendaSidebar({ title, selectedDay, events, onEdit, onEventClick }) {
  return (
    <aside style={S.sidebar}>
      ...
      <div style={S.sidebarBody}>
        {events.length===0 ? (
          <div style={S.emptySidebar}>Nenhuma atividade encontrada para este dia.</div>
        ) : (
          events.map(ev=>(
            <div key={ev.id} style={S.sidebarCard}>
...
const S = {
  ...
  sidebar:{width:"clamp(320px, 28vw, 420px)",flexShrink:0,borderLeft:"1px solid #dadce0",background:"#fbfbfb",display:"flex",flexDirection:"column",minWidth:300},
