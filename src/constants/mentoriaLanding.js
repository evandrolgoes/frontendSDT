export const MENTORIA_LANDING_MODULES = [
  { id: "01", title: "Onboarding", description: "Mentalidade, valores e os pilares da nova profissão." },
  { id: "02", title: "Formação do Preço I", description: "Base física, prêmios e a dinâmica real do campo." },
  { id: "03", title: "Formação do Preço II", description: "Bolsa de Chicago, câmbio e correlações." },
  { id: "04", title: "Macroeconomia", description: "Os vetores globais que movem os preços agrícolas." },
  { id: "05", title: "Mercado Futuro", description: "Long & Short, posição de fundos e dinâmica do mercado." },
  { id: "06", title: "Derivativos I", description: "Opções, NDF e estruturas básicas aplicadas ao físico." },
  {
    id: "07",
    title: "Derivativos II",
    description: "Estratégias avançadas para proteção de margem: ZCC, 3-Way, Call Spread e outros.",
  },
  { id: "08", title: "Análise Gráfica", description: "O uso técnico do gráfico para o timing de decisão." },
  {
    id: "09",
    title: "Política de Hedge",
    description: "Como estruturar uma política de hedge e um comitê de risco profissional.",
  },
  {
    id: "10",
    title: "Execução Prática",
    description: "Operando com a ferramenta SDT Position, a melhor ferramenta de hedge do Brasil.",
  },
  { id: "11", title: "Indicadores Financeiros", description: "As métricas que realmente importam no Agro." },
  {
    id: "12",
    title: "Transição de Carreira",
    description: "O caminho para se tornar um estrategista independente e de valor no mercado.",
  },
  { id: "13", title: "Abordagem Comercial", description: "Construção de carteira e venda de valor." },
  { id: "14", title: "Direcionamento Final", description: "Plantão final e os próximos passos no mercado." },
];

export const MENTORIA_LANDING_PERSONAS = [
  {
    role: "Produtores Rurais",
    context: "Que desejam parar de entregar a margem do ano em decisões emocionais e dominar a trava de preços.",
  },
  {
    role: "Consultores & Agrônomos",
    context: "Que buscam elevar seu nível de entrega, agregando inteligência financeira real ao suporte técnico de campo.",
  },
  {
    role: "Profissionais de Tradings",
    context: "Que operam o físico e precisam entender a fundo a dinâmica dos derivativos para otimizar originação.",
  },
  {
    role: "Investidores do Setor",
    context: "Que buscam compreender os fundamentos reais que movem o preço da saca além da especulação de tela.",
  },
];

export const MENTORIA_LANDING_IMAGES = {
  heroFarm:
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768478680313x884204212029605000/2.%20NDF.pptx%20%281%29.jpg",
  mentorPortrait:
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768478337656x795680711954410200/mentor.JPG",
  abstractBg:
    "https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?q=80&w=1974&auto=format&fit=crop",
  sdtCarousel: [
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768527582763x514806205801388500/1.jpg?_gl=1*1rg55if*_gcl_au*NDE5Njc3MzgwLjE3NjcwMzY1OTc.*_ga*ODA0NTQxODMzLjE3NjcwMzY1OTc.*_ga_BFPVR2DEE2*czE3Njg0NzM5ODQkbzEwJGcxJHQxNzY4NTA1MTI5JGo2MCRsMCRoMA..",
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768527591157x304984797817830400/2.jpg?_gl=1*1rg55if*_gcl_au*NDE5Njc3MzgwLjE3NjcwMzY1OTc.*_ga*ODA0NTQxODMzLjE3NjcwMzY1OTc.*_ga_BFPVR2DEE2*czE3Njg0NzM5ODQkbzEwJGcxJHQxNzY4NTA1MTI5JGo2MCRsMCRoMA..",
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768527599005x485382483897059100/3.jpg?_gl=1*1rg55if*_gcl_au*NDE5Njc3MzgwLjE3NjcwMzY1OTc.*_ga*ODA0NTQxODMzLjE3NjcwMzY1OTc.*_ga_BFPVR2DEE2*czE3Njg0NzM5ODQkbzEwJGcxJHQxNzY4NTA1MTI5JGo2MCRsMCRoMA..",
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768527606044x147461032616644580/4.jpg?_gl=1*1rg55if*_gcl_au*NDE5Njc3MzgwLjE3NjcwMzY1OTc.*_ga*ODA0NTQxODMzLjE3NjcwMzY1OTc.*_ga_BFPVR2DEE2*czE3Njg0NzM5ODQkbzEwJGcxJHQxNzY4NTA1MTI5JGo2MCRsMCRoMA..",
    "https://d6c3507005dbeecfa21c1ba89e6db00d.cdn.bubble.io/f1768527613210x908580389053502100/5.jpg?_gl=1*1rg55if*_gcl_au*NDE5Njc3MzgwLjE3NjcwMzY1OTc.*_ga*ODA0NTQxODMzLjE3NjcwMzY1OTc.*_ga_BFPVR2DEE2*czE3Njg0NzM5ODQkbzEwJGcxJHQxNzY4NTA1MTI5JGo2MCRsMCRoMA..",
  ],
};

export const MENTORIA_LANDING_METRICS = [
  { value: "12+", label: "Anos de Mercado" },
  { value: "+R$500 mm", label: "Gestão de Risco" },
  { value: "R$8 mm +", label: "Ajuste Positivo em Derivativos" },
  { value: "+200 Mil", label: "Contratos Negociados" },
  { value: "+3,5 mm", label: "Sacas de Soja sob Gestão" },
];
